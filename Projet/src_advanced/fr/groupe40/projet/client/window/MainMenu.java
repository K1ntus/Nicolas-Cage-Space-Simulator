...
public class MainMenu {
	...
	public void init() {
		...
		//Background
		GridElement.add_image_to_gridpane(ResourcesContainer.getMain_menu_background(), Constants.width/5, true, 3, 6, grid);
		...
		grid.setBackground(new Background(new BackgroundImage(new Image((Resources.path_img_menu_background)), null,
				...
				new BackgroundSize(Constants.width, 0.0, true, false, false, true))));
	...
	private void display_title(GridPane grid) {
		...
		title_img.setFitWidth(Constants.width / 2);
		title_img.setPreserveRatio(true);
		grid.add((title_img), 2, 0);
		title_img.setTranslateX(Constants.width / 5);
	...
	public Scene getScene() {
		Scene res = new Scene(grid, Constants.width, Constants.height);
