...
public class Squad implements Serializable {
	...
	public void sendFleet() {
		...
		while (summoning) {
			...
			case TOP:
				...
				y = source.getY() - Constants.size_squads * 1.5;
				break;
			case BOTTOM:
				x = this.horizontalSummoning();
				y = source.getY() + source.height() + 0.5 * Constants.size_squads;
				break;
			case LEFT:
				y = this.verticalSummoning();
				x = source.getX() - Constants.size_squads * 1.5;
				break;
			case RIGHT:
				y = this.verticalSummoning();
				x = source.getX() + source.width() + 0.5 * Constants.size_squads;
				break;
			default:
			...
			ships.add(new Ship(img_path, ruler, destination, source, x, y, source.getShips_type()));
	...
	private double verticalSummoning() {
		...
		if (summonY * Constants.size_squads + source.getY() < source.getY() + source.height()) {
			summonY += 1; // Reset summon y

		} else if (summonY * Constants.size_squads + source.getY() >= source.getY() + source.height()) {
			summonY = 1;
			if (summonX != -1)
				summonX = -1;
			if (summonX == -1) {
		...
		return (summonY * Constants.size_squads + (source.getY() - Constants.size_squads));
	}
	...
	private Direction summoningSide() {
		...
		if (Sprite.distance(source.getX() + source.width() / 2, source.getY(),
				destination.getX() + destination.width() / 2, destination.getY() + destination.height() / 2) <= min) {
			min = Sprite.distance(source.getX() + source.width() / 2, source.getY(),
					destination.getX() + destination.width() / 2, destination.getY() + destination.height() / 2);
			res = Direction.TOP;
		}

		if (Sprite.distance(source.getX() + source.width() / 2, source.getY() + source.height(),
				destination.getX() + destination.width() / 2, destination.getY() / 2) <= min) {
			min = Sprite.distance(source.getX() + source.width() / 2, source.getY() + source.height(),
					destination.getX() + destination.width() / 2, destination.getY() / 2);
			res = Direction.BOTTOM;
		}

		if (Sprite.distance(source.getX(), source.getY() + source.height() / 2,
				destination.getX() + destination.width() / 2, destination.getY() + destination.height() / 2) <= min) {
			min = Sprite.distance(source.getX(), source.getY() + source.height() / 2,
					destination.getX() + destination.width() / 2, destination.getY() + destination.height() / 2);
			res = Direction.LEFT;
		}

		if (Sprite.distance(source.getX() + source.width(), source.getY() + source.height() / 2,
				destination.getX() + destination.width() / 2, destination.getY() + destination.height() / 2) <= min) {
			min = Sprite.distance(source.getX() + source.width(), source.getY() + source.height() / 2,
					destination.getX() + destination.width() / 2, destination.getY() + destination.height() / 2);
			res = Direction.RIGHT;
		}
	...
	/**
	 * Get the ruler of the squad (by getting the ruler of the first element of the
	 * squad's ship array)
	 * 
	 * @return the ruler
	 */
